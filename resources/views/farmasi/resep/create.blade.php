...
<form action="{{ route('resep.store') }}" method="POST" class="card p-3">
    @csrf

    {{-- Header resep (bisa diintegrasikan dengan data dari klinik) --}}
    <div class="row mb-3">
        <div class="col-md-4">
            <label class="form-label">Jenis Resep</label>
            <input type="text" name="jenis_resep" class="form-control" value="{{ old('jenis_resep') }}" required>
        </div>
        <div class="col-md-4">
            <label class="form-label">ID Pasien (opsional)</label>
            <input type="number" name="id_pasien" class="form-control" value="{{ old('id_pasien') }}">
        </div>
        <div class="col-md-4">
            <label class="form-label">ID Dokter (opsional)</label>
            <input type="number" name="id_dokter" class="form-control" value="{{ old('id_dokter') }}">
        </div>
    </div>

    <div class="mb-3">
        <label class="form-label">Catatan Dokter (opsional)</label>
        <textarea name="catatan_dokter" class="form-control" rows="2">{{ old('catatan_dokter') }}</textarea>
    </div>
